...
**287. Restaking Operator Collusion via Shared AVS Quorum Manipulation**

- **D:** In restaking protocols (EigenLayer/Symbiotic), a set of operators registered to the same AVS can collude to exceed the quorum threshold required for malicious attestations. Because the same restaked ETH backs all AVS registrations, operators can simultaneously satisfy quorum requirements across multiple AVSs while maintaining minimal unique stake. A coordinated attestation of an invalid state (e.g., fraudulent bridge message, incorrect oracle price) passes the quorum check and is finalized before the fraud proof window expires. The slashing penalty is capped per operator, so the extractable value from the fraudulent attestation exceeds the total slashable stake. Pattern: `quorumReached = totalStakeFor > threshold` where `totalStakeFor` counts operators whose individual stake is shared across AVSs without pro-rata attribution.
- **FP:** Protocol uses unique-stake attribution: each unit of restaked ETH can only count toward one AVS quorum at a time. Quorum threshold accounts for maximum correlated slashing exposure. Economic security module enforces `extractableValue < totalSlashableStake` invariant.

**288. Intent Solver Backrunning via Delayed Fill with Stale Pricing**

- **D:** In intent-based DEX protocols (UniswapX, CoW Protocol), a solver receives a user's signed intent specifying a minimum output amount. The solver delays filling the intent until just before the deadline while monitoring price movements. If the price moves favorably for the solver after the user signed, the solver fills at the user's stale minimum price and captures the surplus. Because intents have validity windows (often 5-30 minutes), the solver has a free option on price direction. Pattern: `require(block.timestamp <= order.deadline)` with `require(amountOut >= order.minAmountOut)` where `minAmountOut` was computed at signing time, not fill time.
- **FP:** Dutch auction decay mechanism reduces the solver's fill price over time, returning surplus to the user. Protocol enforces competitive solver auctions with on-chain price improvement verification. Fill price must beat a reference oracle price, not just the user's minimum.

**289. EIP-7702 Delegated EOA Approval Persistence After Revocation**

- **D:** An EOA delegates to a smart contract wallet via EIP-7702 and issues ERC-20 `approve()` calls through the delegated code. When the EOA later revokes the delegation (sets delegation to `address(0)` or a new target), the token approvals granted during delegation persist because `approve()` writes to the token contract's storage, not the EOA's transient delegation state. An attacker who obtained a stale approval can call `transferFrom()` against the EOA's token balance even after the delegation is revoked. The EOA owner believes revoking delegation revokes all permissions, but token-level approvals are independent of EIP-7702 delegation state. Pattern: `token.allowance(eoaAddress, spender)` remains non-zero after `eoaAddress` revokes its 7702 delegation.
- **FP:** Protocol tracks approvals in the delegated contract's own namespaced storage (ERC-7201) rather than on external token contracts. Delegation revocation handler explicitly revokes all known token approvals. User wallet UI shows token-level approvals as a separate security surface from delegation.

**290. Restaking Withdrawal Queue Griefing via Minimum Unstake Spam**

- **D:** Restaking protocols implement withdrawal queues where each unstake request creates an entry that must be processed sequentially or in batches. An attacker creates thousands of minimum-size withdrawal requests (e.g., 1 wei of restaked ETH each), congesting the queue and delaying legitimate large withdrawals. The queue processing function iterates over entries with `O(n)` gas cost, eventually hitting the block gas limit and preventing any withdrawals from being processed. Pattern: `withdrawalQueue.push(WithdrawalRequest({amount: minAmount, ...}))` in a loop with no per-user queue depth limit.
- **FP:** Protocol enforces minimum withdrawal amount significantly above dust (e.g., 0.01 ETH). Per-user queue depth limit prevents spam. Queue uses a bitmap or priority structure that processes in `O(1)` per entry. Withdrawal fee proportional to queue entries discourages spam economically.

**291. Cross-Domain MEV via Intent Composability Between L1 and L2**

- **D:** User submits an intent on L2 that, when filled, triggers a cross-domain effect on L1 (e.g., bridging tokens, closing a position, claiming rewards). A sophisticated solver observes the L2 intent in the mempool, front-runs the consequent L1 action by pre-positioning on L1 before the L2 intent is filled, and captures the cross-domain price impact. Because L1 and L2 have different block times and sequencer models, the solver has an information advantage: they see the L2 intent but can act on L1 before the intent's L1 effects land. Pattern: L2 intent fill triggers `L1Bridge.finalizeWithdrawal()` which swaps tokens on L1 — solver sandwiches the L1 swap knowing it's coming from the L2 intent fill.
- **FP:** Intent protocol uses atomic cross-domain settlement where L1 and L2 legs execute in the same effective block. Solver competition with on-chain price improvement requirements eliminates cross-domain surplus capture. Protocol uses private intent submission channels that prevent mempool observation.
